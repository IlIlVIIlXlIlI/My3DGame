...
cbuffer global_Line:register(b2)
{
	matrix g_mRayWVP;
	float4 g_Color;
};
...
{	
	...
	return  input.Color/2+TexDiffuse;
}

//
//レイ用 バーテックスシェーダー
//
float4 RayVS(float4 Pos : POSITION) : SV_POSITION
{
...
//レイ用 ピクセルシェーダー
//
float4 RayPS(float4 Pos : SV_POSITION) : SV_Target
{
	return g_Color;
}