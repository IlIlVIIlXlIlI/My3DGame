...
HRESULT CD3DXMESH_ANIM::CreateAppMeshFromD3DXMesh(LPD3DXFRAME p)
{
	...
	for( DWORD i=0; i<pAppMesh->dwNumMaterial; i++ )
	{
		...
		//テクスチャーがあれば
		if( pFrame->pMeshContainer->pMaterials[i].pTextureFilename != nullptr &&
...
HRESULT CD3DXMESH_ANIM::InitShader()
{
	...
	if(FAILED(m_pDevice11->CreateVertexShader(pCompiledShader->GetBufferPointer(),pCompiledShader->GetBufferSize(),nullptr,&m_pVertexShader)))
	{
		...
		MessageBox(0,"バーテックスシェーダー作成失敗(CD3DXMESH_ANIM)",nullptr,MB_OK);
		return E_FAIL;
	}
	//頂点インプットレイアウトを定義	
	...
	if(FAILED(m_pDevice11->CreateVertexShader(pCompiledShader->GetBufferPointer(),pCompiledShader->GetBufferSize(),nullptr,&m_pVertexShaderNoTex)))
	{
		SAFE_RELEASE(pCompiledShader);
		MessageBox(0,"バーテックスシェーダー作成失敗(CD3DXMESH_ANIM)",nullptr,MB_OK);
		return E_FAIL;
	...
	//ブロブからピクセルシェーダー作成
	if(FAILED(D3DX11CompileFromFile("MESH.hlsl",nullptr,nullptr,"PS","ps_5_0",0,0,nullptr,&pCompiledShader,&pErrors,nullptr)))
	{
		MessageBox(0,"hlsl読み込み失敗",nullptr,MB_OK);
		return E_FAIL;
	}
	...
	//テクスチャーなし
	if(FAILED(D3DX11CompileFromFile("MESH.hlsl",nullptr,nullptr,"PS_NoTex","ps_5_0",0,0,nullptr,&pCompiledShader,&pErrors,nullptr)))
	{
		MessageBox(0,"hlsl読み込み失敗",nullptr,MB_OK);
		return E_FAIL;
	}
	SAFE_RELEASE(pErrors);
	if(FAILED(m_pDevice11->CreatePixelShader(pCompiledShader->GetBufferPointer(),pCompiledShader->GetBufferSize(),nullptr,&m_pPixelShaderNoTex)))
	{
		SAFE_RELEASE(pCompiledShader);
		MessageBox(0,"ピクセルシェーダー作成失敗",nullptr,MB_OK);
		return E_FAIL;
...
void CD3DXMESH_ANIM::DrawPartsMesh(PARTS_MESH* pPartsMesh,D3DXMATRIX World)
{
	...
	else
	{
		...
		//使用するシェーダーの登録
		m_pDeviceContext11->VSSetShader(m_pVertexShaderNoTex,nullptr,0);
		m_pDeviceContext11->PSSetShader(m_pPixelShaderNoTex,nullptr,0);
	}
	//バーテックスバッファーをセット
	UINT offset = 0;
