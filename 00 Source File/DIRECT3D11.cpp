...
HRESULT DIRECT3D11::Init(D3D_INIT* pcd)
{
	...
	m_pDevice->CreateTexture2D( &descDepth, nullptr, &m_pBuckBuffer_DSTex );
	//そのテクスチャーに対しデプスステンシルビュー(DSV)を作成
	m_pDevice->CreateDepthStencilView( m_pBuckBuffer_DSTex, nullptr, &m_pBuckBuffer_DSTexDSV);
	...
	m_pDeviceContext->OMSetBlendState(m_pBlendState, nullptr, mask);

#ifdef _DEBUG
	//文字列レンダリングの初期化
	m_pText = new D3D11_TEXT;
	D3DXVECTOR4 vColor(1.0, 1.0, 1.0, 1.0);
	if (FAILED(m_pText->Init(m_pDeviceContext, WINDOW_WIDTH, WINDOW_HEIGHT, 25, vColor)))
	{
		return E_FAIL;
	}
...
void DIRECT3D11::ShowFPS()
{
	...
	/*=====================*/
#ifdef _DEBUG
	sprintf_s(sz, "%d fps", FPS);
	m_pText->Render("DEBUG MODE", 0, 10);
	m_pText->Render(sz, 0, 40);
	Frames++;
#endif
}
